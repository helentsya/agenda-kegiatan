        ...
        <script>
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = "{{ route('bidang.cuti.index') }}";
                }
            });
        </script>
    @endif
    <main id="main-content" class="bg-gray-100 ml-56 p-4 sm:p-8 md:p-12  lg:p-16 pt-8 min-h-screen">
        <form class="py-4 px-8 bg-white rounded-md shadow-md" action="{{ route('bidang.cuti.store') }}" method="post">
            @csrf
            ...
            <div class="relative mb-4 w-full">
                <input required type="date" id="floating_awal_cuti"
